...
1. Add a public right shift operator `operator>>` to `CardSet` to move one card from the source CardSet to the destination CardSet. We will use this operator to deal cards from the deck to a player's hand.
